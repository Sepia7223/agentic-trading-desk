...
from trading_desk.strategy import indicators as I
...
def score_momentum(ind: dict) -> tuple[int, str]:
    ...
    if rsi is not None:
        if rsi >= 55:
            pts += 1
            bits.append(f"RSI {rsi:.0f}≥55")
        elif rsi <= 45:
            pts -= 1
            bits.append(f"RSI {rsi:.0f}≤45")
        else:
    ...
    if trix is not None and trix_sig is not None:
        if trix > trix_sig and trix > 0:
            pts += 1
            bits.append("TRIX>signal>0")
        elif trix < trix_sig and trix < 0:
            pts -= 1
            bits.append("TRIX<signal<0")
        else:
...
def _flags(ind: dict) -> dict:
    ...
    if (
        e20
        and c > e20
        and ind["ema20_slope"] is not None
        and ind["ema20_slope"] > 0
        and bsb is not None
        and 1 <= bsb <= 5
    ):
    ...
    if (
        trix is not None
        and trix_sig is not None
        and trix_p is not None
        and sig_p is not None
        and trix > trix_sig
        and trix_p <= sig_p
        and trix <= 0
    ):
...
def decide(ind: dict, trend: int, mom: int, macro: int | None, holding: bool | None) -> dict:
    """
    ...
    if holding is None:
        return {
            "action": "NO TRADE",
            "rationale": "Position state unknown.",
            "framing": "No trade: resolve position state before evaluating entry or exit actions.",
            "flags": f,
        }

    in_pos = holding is True
    ...
    if in_pos and n_exh >= 2:
        ...
        if n_reb >= 2:
            framing += (
                " Rebound in progress: use it to exit at a better price, not to justify holding."
            )
    elif not in_pos and n_reb >= 2 and not dc:
        action = "RE-ENTRY (new cycle)"
        rationale = (
            "Rebound/reversal with healthy EMA structure: likely start of a new bullish cycle."
        )
        framing = (
...
def render(card: dict) -> str:
    ...
    L.append(
        line(
            "Macro-Sentiment",
            ms,
            p["macro_sentiment"]["detail"] if ms is not None else "(not injected)",
        )
    )
    L.append(f"  {'─' * 50}")
...
def main() -> int:
    ...
    ap.add_argument(
        "input",
        nargs="?",
        help="JSON: {symbol, close:[...], macro_score?, holding?}. Without file: self-test.",
    )
