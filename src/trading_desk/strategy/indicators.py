...
def _strip(values: list[float | None]) -> list[float]:
    return [v for v in values if v is not None]


def rsi_wilder(close: list[float], period: int = 14) -> list[float | None]:
...
def macd(close: list[float], fast: int = 12, slow: int = 26, signal: int = 9):
    ...
    line: list[float | None] = [
        (a - b) if (a is not None and b is not None) else None for a, b in zip(ef, es, strict=False)
    ]
    valid = _strip(line)
    sig_valid = ema_series(valid, signal)
    # re-align signal to original length
    sig: list[float | None] = [None] * len(close)
    ...
    hist: list[float | None] = [
        ...
        for m, s in zip(line, sig, strict=False)
    ]
...
def _slope(series: list[float | None], lookback: int) -> float | None:
    ...
    last_v = series[last_i]
    prev_v = series[prev_i]
    if last_v is None or prev_v is None:
        return None
...
def compute(close: list[float], slope_lookback: int = 5) -> dict:
    ...
    for back in range(len(close)):
        i = len(close) - 1 - back
        ema20_value = ema20[i]
        if ema20_value is not None and close[i] < ema20_value:
    ...
    return {
        ...
        "trix": last(trix_line),
        "trix_prev": prev(trix_line),
        "trix_signal": last(trix_sig),
        "trix_signal_prev": prev(trix_sig),
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        description="Deterministic indicator stack (input: JSON of closes)."
    )
    ap.add_argument(
        "input", nargs="?", help="JSON: {'close':[...]} or [..]. If no file: self-test."
    )
