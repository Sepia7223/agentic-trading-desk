...
def ratio_series(num: list[float], den: list[float]) -> list[float]:
    ...
    num, den = num[-n:], den[-n:]
    return [a / b for a, b in zip(num, den, strict=False) if b != 0]
...
class Component:
    ...
    ratio: str
    weight: float
    signal: float | None = None  # -1 / 0 / +1 (risk-on oriented)
    detail: str = ""
    available: bool = True


def trend_signal(
    series: list[float], fast: int, slow: int, slope_win: int
...
def score_macro(
    data: dict, fast: int = 50, slow: int = 200, slope_win: int = 20, corr_win: int = 40
) -> MacroResult:
    series = data.get("series", {})
    notes: list[str] = []

    def closes(sym: str) -> list[float] | None:
        v = series.get(sym)
        if not v:
            return None
    ...
    c = Component(
        "Concentration (equal vs cap-weight)", "RSP/SPY", BASE_WEIGHTS["concentration"][1]
    )
    if rsp and spy:
    ...
    if spread and len(spread) >= slope_win + 1:
        ...
        c.available = False
        notes.append(
            "No yield_spread: redistributing the 20% weight of the curve among other components."
        )
    comps["yield_curve"] = c
    ...
    for component in avail:
        if component.signal is not None:
            weighted_sum += component.signal * component.weight
    composite = weighted_sum / wsum
    composite = clamp(composite, -1.0, 1.0)

    # --- Inflationary flag: positive SPY-TLT correlation + weak equity-bond ---
    eb = comps["equity_bond"]
    inflationary = bool(
        spy_tlt_corr is not None
        and spy_tlt_corr > 0.25
        and eb.available
        and eb.signal is not None
        and eb.signal <= 0
    )
...
def render(r: MacroResult) -> str:
    ...
    if r.spy_tlt_corr is not None:
        L.append(
            f"SPY-TLT Corr   : {r.spy_tlt_corr:+.3f}"
    ...
    for c in r.components:
        if c["available"] and c["signal"] is not None:
            ...
            L.append(
                f"  {arrow} {c['ratio']:<9} w={c['weight']:.2f}  "
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Macro-Sentiment pillar scorer (cross-asset, Robinhood-compatible)."
    )
    ap.add_argument(
        "input",
        nargs="?",
        help="JSON with close price series. If omitted, runs a synthetic self-test.",
    )
    ap.add_argument(
        "--json",
        action="store_true",
        help="Output in JSON format instead of human-readable report.",
    )
    ...
    r = score_macro(
        data, fast=args.fast, slow=args.slow, slope_win=args.slope_win, corr_win=args.corr_win
    )
